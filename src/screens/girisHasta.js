...
const GirisHasta = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    const ok = await loginUser(email.trim().toLowerCase(), sifre);

    if (!ok) {
      Alert.alert("Hata", "E-posta veya şifre hatalı (ya da backend erişilemiyor)");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View style={styles.box}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate("kayitHasta")} activeOpacity={0.7}>
